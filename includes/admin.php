...
function fp5_globalOptionsPage() {
    ...
    <div id="fp5_global" class="wrap">
        <div id="flowplayer5-icon" style="background: url('http://flowplayer.org/favicon.ico') no-repeat;" class="icon32">
            <br>
        </div>
        ...
        <form method="post" action="admin-post.php">
            ...
            <div class="options">
                <div clss="optgroup separated">
                    <div class="head" for="fp5_licenseKey">
                        <h3><?php _e('Commercial version') ?></h3>
                        <?php printf( __('Commercial version removes the Flowplayer logo and allows you to use your own logo image. You can obtain and purchase a license key at <a%s>flowplayer.org</a>.'),
                    ...
                    <div class="option wide">
                        ...
                        <p class="checkpox">
                            <?php printf( __('Uncheck this and the logo is only shown in <a%s>virally embedded players</a>'),
                            ' href="http://flowplayer.org/docs/#custom-logo" target="_blank" '
                            )?>
                        </p>
                    </div>
                </div>
                ...
                <div class="optgroup separated">
                    <div class="head" for="fp5_analytics">
                        <h3><?php _e('Tracking') ?></h3>
                        <?php _e('You can track video traffic using Google Analytics (GA). Specify your GA account ID here.') ?>
                        <a href="http://flowplayer.org/docs/analytics.html" target="_blank"><?php _e('Visit flowplayer.org for more info') ?></a>.
                    </div>
                ...
                <div class="optgroup separated">
                    <div class="head" for="fp5_cdn">
                        <h3><?php _e('CDN Option') ?></h3>
                        <?php _e('If you want to use a self-hosted copy of Flowplayer instead of the CDN hosted version uncheck <em>Use CDN hosted version?</em>. <strong>Using the CDN hosted version is preferable in most situations.</strong>')?>
                    </div>
                    <div class="option wide" id="fp5_cdn">
                        <label for="flowplayer_cdn"><?php _e('Use CDN hosted version?')?></label>
                        <input type="checkbox" id ="flowplayer_cdn" name="flowplayer_cdn" value="true" <?php echo $options['flowplayer_cdn'] ? "checked=\"true\"" : ""?> />
                    </div>
                </div>
            </div>
...
function fp5_initGlobalOptions() {
    $options = get_option('fp5_options');
    if ($options == false) {
        $new_options['flowplayer_cdn'] = true;
        ...
        if (! isset($options['flowplayer_cdn'])) {
            $options['flowplayer_cdn'] = true;
            update_option('fp5_options', $options);
        }
    }
}
...
function fp5_saveGlobalOptions() {
    ...
    if(isset($_POST['flowplayer_cdn']) && $_POST['flowplayer_cdn'] == "true") {
        $options['flowplayer_cdn'] = true;
    } else {
        $options['flowplayer_cdn'] = false;
    }
...
function fp5_handleAdminMenu() {
    ...
    // an admin page for specifying options
    add_options_page('Flowplayer options', 'Flowplayer', 'manage_options', 'fp5_options', 'fp5_globalOptionsPage');
}
